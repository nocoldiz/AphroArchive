...
function _actorCard(a) {
  ...
  return '<div class="actor-card fade-in' + (a.count === 0 ? ' actor-card-unmatched' : '') + '" onclick="openActor(\'' + escA(a.name) + '\')">' +
...
function renderActors(actors) {
  ...
  const active = actors.filter(a => a.count > 0);
  const others = actors.filter(a => a.count === 0);
  ...
  let html = '';
  if (active.length) html += '<div class="actor-grid">' + active.map(_actorCard).join('') + '</div>';
  if (others.length) {
    html += '<div class="actor-section-sep"><span>Other Actors</span></div>';
    html += '<div class="actor-grid">' + others.map(_actorCard).join('') + '</div>';
  }
  el.innerHTML = html;
}
