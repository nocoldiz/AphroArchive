...
function renderCategoriesView() {
  ...
  grid.innerHTML = items.map(item => {
    ...
    const onctx  = item.type === 'cat'
      ? ' oncontextmenu="showContextMenu(event, \'category\', {path:\'' + escA(item.path) + '\', name:\'' + escA(item.name) + '\', encrypted:' + !!item.encrypted + '})"'
      : '';
    const lockIcon = item.encrypted ? '<svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" style="position:absolute;top:10px;right:10px;z-index:2;color:white;filter:drop-shadow(0 2px 4px rgba(0,0,0,0.5))"><rect x="3" y="11" width="18" height="11" rx="2"/><path d="M7 11V7a5 5 0 0 1 10 0v4"/></svg>' : '';
