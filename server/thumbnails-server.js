...
// A quiet stand-in for videos with no generated thumbnail. Serving this at
// 200 instead of a 404 keeps the browser console clean (grids request a thumb
// for every visible card) while `no-store` lets the real image replace it as
// soon as generation completes.
const THUMB_PLACEHOLDER_SVG = Buffer.from(
  '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 480 270">'
  + '<rect width="480" height="270" fill="#1a1a1e"/>'
  + '<g fill="none" stroke="#3a3a42" stroke-width="6"><rect x="200" y="103" width="80" height="64" rx="8"/><path d="M212 155l16-20 12 14 10-10 18 16"/></g>'
  + '</svg>'
);

function sendThumbPlaceholder(res) {
  res.writeHead(200, { 'Content-Type': 'image/svg+xml', 'Cache-Control': 'no-store' });
  res.end(THUMB_PLACEHOLDER_SVG);
}
...
async function apiThumbGen(req, res, id) {
  ...
  if (cache[id] && cache[id].mtime === stat.mtimeMs && !force) {
    const c = cache[id];
    if (c.count > 0)
      return json(res, { count: c.count, duration: c.duration || null, width: c.width || null, height: c.height || null });
    // Known-bad: generation already failed for this exact file version.
    // Don't re-spawn ffmpeg on every card view — corrupted files used to be
    // retried forever, flooding the log. Retry via ?force=1 or Generate All.
    return json(res, { count: 0, failed: true });
  }
...
async function apiThumbImg(req, res, id, idx) {
  ...
  if (!fs.existsSync(fp)) {
    ...
    sendThumbPlaceholder(res); return;
  }
  res.writeHead(200, { 'Content-Type': 'image/jpeg', 'Cache-Control': 'public, max-age=604800' });
...
async function apiThumbnailsList(req, res) {
  try {
    ...
    // Answer from the SQLite thumbs cache — walking every thumb directory on
    // disk took ~18 s on a large library. Fall back to the fs scan only when
    // the cache is empty (e.g. thumbs generated by a pre-cache version).
    const cache = loadThumbsCache();
    const cachedIds = Object.keys(cache).filter(id => cache[id] && cache[id].count > 0);
    const results = [];
    if (cachedIds.length > 0) {
      for (const id of cachedIds) {
        if (visibleIds && !visibleIds.has(id)) continue;
        const count = cache[id].count;
        results.push({
          id,
          count,
          thumbs: Array.from({ length: count }, (_, i) => `/api/thumbs/${id}/${i}`)
        });
      }
    } else {
